...
# Define sanitized bucket name as a global variable
...
echo "Starting GCP setup script for bucket $GCP_BUCKET_APP_NAME renamed to $SANITIZED_BUCKET_NAME"
exit 1
...
gcloud dns record-sets transaction add --zone="$GCP_DNS_ZONE_NAME" \
  --name="$DOMAIN_NAME" --ttl=300 --type=A "$STATIC_IP"
...
gcloud storage buckets add-iam-policy-binding "gs://${SANITIZED_BUCKET_NAME}" \
  --member=allUsers \
  --role=roles/storage.objectViewer

gcloud storage buckets update "gs://${SANITIZED_BUCKET_NAME}" \
