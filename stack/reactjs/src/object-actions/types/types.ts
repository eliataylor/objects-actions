...
export function getProp<T extends EntityTypes, K extends keyof T>(
  entity: EntityTypes,
  key: string,
): T[K] | null {
  // @ts-ignore
...
export function restructureAsAllEntities(
  modelName: keyof typeof TypeFieldSchema,
  entity: any,
): any {
    ...
    if (field.data_type === 'RelEntity') {
      if (!value) {
        ...
        result[key] = value.map((item) =>
          item.entity
            ? restructureAsAllEntities(item._type, item.entity)
            : item,
        );
      } else if (value.entity) {
        // Transform a single RelEntity
        result[key] = value?.entity
          ? restructureAsAllEntities(value._type, value.entity)
          : value;
      } else {
