...
const FormWrapper: React.FC<FormWrapperProps> = ({ onSubmit, entityData }) => {
  ...
  const renderLinkField = (
    ...
    return (
      <Grid
        key={`${field.machine}_${index}_${sourceName}`}
        item
        xs={12}
        sm={6}
        md={4}
      >
        <FormControl fullWidth>
          ...
          <Input
            ...
            onFocus={() =>
              console.log('onFocused', sourceName, field.machine, entry)
            } // Implement onFocused logic
    ...
    <Grid container spacing={2}>
      <form onSubmit={handleSubmit}>
        ...
        {fields?.map((field, index) =>
          renderLinkField(field, entityData, index, 'sourceName'),
        )}
        <Grid item xs={12}>
