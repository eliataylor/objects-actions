...
class VerifyCodeView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if str(request.session.get('code')) == code:

                redirect_url = f"{settings.FRONTEND_URL}/auth/sms/verify"

                user, created = get_user_model().objects.get_or_create(username=phone_number,
                                                           defaults={'email': f'{phone_number}@sms-placeholder.com'})
                if created:
                ...
                session = LoginSession(request, "sms_login_session", settings.SESSION_COOKIE_NAME)
                response = JsonResponse({"detail": "Verification successful", "id": user.id, "redirect": redirect_url}, status=status.HTTP_200_OK)
                response.url = redirect_url
                session.save(response)
                return response

            return JsonResponse({"detail": "Invalid code"}, status=status.HTTP_400_BAD_REQUEST)
